...
const ViewDoctor = (props) => {
  ...
  // Nếu không có lịch trình, không hiển thị gì
  ...
  const handleToggleExpand = () => {
    ...
    if (!isExpanded) {
      setVisibleCount(dataDetailDoctor.thoiGianKham.length);
    } else {
      setVisibleCount(2);
    }
  };
  ...
  const handleDeleteDateCu = async () => {
    try {
      const response = await xoaLichCu(dataDetailDoctor._id);
  ...
  const items = [
    ...
    },
    {
      ...
      children: (
        <Badge status="processing" text={`${dataDetailDoctor?.roleId.key}`} />
      ),
      span: 2,
    },
    {
      ...
      children: (
        <>
          {dataDetailDoctor?.chucVuId.map((value, index) => (
            <div key={value._id}>
              {value.name}
              {index < dataDetailDoctor.chucVuId.length - 1 && <br />}{" "}
            </div>
          ))}
        </>
      ),
      ...
      children: (
        <>
          {dataDetailDoctor?.chuyenKhoaId.map((value, index) => (
            <div key={value._id}>
              {value.name}
              {index < dataDetailDoctor.chuyenKhoaId.length - 1 && <br />}{" "}
            </div>
          ))}
        </>
      ),
